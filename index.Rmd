...
food.df = read.xlsx(food.file.path, sheet = 1)

# manually matched foods between two databases
water.to.food = read.csv("water_to_food.csv", colClasses = "character")
...
## Bubble chart
water.food.bubble = gvisBubbleChart(all.df, 
	idvar="custom.descrip.x", xvar="gprotein.per.gal.us", yvar="kcal.per.gal.us",
	colorvar="custom.categ.x", sizevar="kcal.per.gal.us",
	options=list(
	# hAxis='{minValue:75, maxValue:125}',
	width=750, height=650))
plot(water.food.bubble)

water.food.bar = gvisBarChart(all.df, 
	idvar="custom.descrip.x", xvar="gprotein.per.gal.us", yvar="kcal.per.gal.us",
	colorvar="custom.categ.x", sizevar="kcal.per.gal.us",
	options=list(
	# hAxis='{minValue:75, maxValue:125}',
...
```{r kcalpergal, fig.cap="", fig.height=5, fig.width=7, dpi=300, fig.align='center', message=FALSE, echo=FALSE, cache=FALSE, eval=FALSE}
all.df = transform(all.df, custom.descrip.x = reorder(custom.descrip.x, kcal.per.gal.us))

ggplot(subset(all.df, !is.na(kcal.per.gal.us)), 
 		aes(x = custom.descrip.x, fill = custom.categ.x)) +
 	geom_bar(stat="identity", ymin=0, width=0.75, 
 		aes(y=kcal.per.gal.us, ymax=kcal.per.gal.us), position="dodge") +
    geom_text(aes(x=custom.descrip.x, y=kcal.per.gal.us, label=custom.descrip.x), size=2, hjust=-0.1) +
 	# scale_y_continuous("Calories per gallon of water (U.S. average)", expand = c(0, 0)) +
 	# xlab("") +
 	scale_y_sqrt("Calories per gallon of water (U.S. average)") +
...
```{r galperkcal, fig.cap="", fig.height=7, fig.width=6, dpi=300, fig.align='center', message=FALSE, echo=FALSE, cache=FALSE}
all.df = transform(all.df, custom.descrip.x = reorder(custom.descrip.x, kcal.per.gal.us))
...
ggplot(subset(all.df, !is.na(gal.per.kcal.us)), 
    ...
    geom_text(aes(x=custom.descrip.x, y=gal.per.kcal.us, label=custom.descrip.x), size=2, hjust=-0.1) +
 	xlab("") +
 	scale_y_continuous("Gallons of water per Calorie", 
 		expand = c(0, 0),
 		limits = c(0, max(all.df$gal.per.kcal.us,na.rm=TRUE)+0.35)) +
 	# scale_fill_brewer("", palette="Set2") +
...
```{r galperkcalcali, fig.cap="", fig.height=7, fig.width=6, dpi=300, fig.align='center', message=FALSE, echo=FALSE, cache=FALSE}
...
ggplot(subset(all.df, !is.na(gal.per.kcal.cali)), 
    ...
    geom_text(aes(x=custom.descrip.x, y=gal.per.kcal.cali, label=custom.descrip.x), size=2, hjust=-0.1) +
 	xlab("") +
 	scale_y_continuous("Gallons of water per Calorie", 
 		expand = c(0, 0),
 		limits = c(0, max(all.df$gal.per.kcal.cali,na.rm=TRUE)+0.35)) +
	scale_fill_hue("", h=c(30, 300), c = 65, l = 75, h.start=125,
		drop=TRUE,
...
```{r galperkcalglobal, fig.cap="", fig.height=7, fig.width=6, dpi=300, fig.align='center', message=FALSE, echo=FALSE, cache=FALSE}
...
ggplot(subset(all.df, !is.na(gal.per.kcal.global)), 
    ...
    geom_text(aes(x=custom.descrip.x, y=gal.per.kcal.global, label=custom.descrip.x), size=2, hjust=-0.1) +
 	xlab("") +
 	scale_y_continuous("Gallons of water per Calorie", 
 		expand = c(0, 0),
 		limits = c(0, max(all.df$gal.per.kcal.global,na.rm=TRUE)+0.35)) +
 	# scale_fill_brewer("", palette="Set2") +
	scale_fill_hue("", h=c(30, 300), c = 65, l = 75, h.start=125) +
 	coord_flip() + theme_classic() +
 	theme(axis.text.y = element_blank(),
...
p = nPlot(gal.per.kcal.us ~ custom.descrip.x, group = 'custom.categ.x', data = subset(all.df, !is.na(gal.per.kcal.us)), type = "multiBarHorizontalChart", height = 1000, width = 800)
p$yAxis(axisLabel = "Gallons of water per Calorie")
p$templates$script = "http://timelyportfolio.github.io/rCharts_nvd3_templates/chartWithTitle.html"
p$set(title = "Average gallons of water required in the U.S. to produce one Calorie, listed by food type")
...
Note that, Ercin, Aldaya, & Hoekstra (2012), feedback received on twitter, and the [LA Times' correction on 12 April 2015](http://www.latimes.com/local/corrections/la-a4-correx-20150412-story.html), the figure for U.S. average gallons of water required to produce 100 grams of soy milk has been manually corrected from 84.92338 gal/100g to 7.580173 gal/100g.  This figure is for global, not U.S., production (China, France, and Canada).
...
Ercin, A.E., Aldaya, M.M. and Hoekstra, A.Y. (2012) The water footprint of soy milk and soy burger and equivalent animal products, Ecological Indicators, 18: 392−402. Available at: http://waterfootprint.org/media/downloads/Ercin-et-al-2012-WaterFootprintSoy_1.pdf
...
Alexander Walker (2015). openxlsx: Read, Write and Edit XLSX Files. R package version 2.4.0. http://CRAN.R-project.org/package=openxlsx
